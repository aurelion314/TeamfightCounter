...
function addon:updateGroups()
    ...
    for i, group in pairs(groups) do
        ...
        -- if not group.zone then
        --     for j, player in pairs(group.players) do
        --         if player.zone then
        --             group.zone = player.zone
        --             break
        --         end
        --     end
        -- end
...
function addon:showGroupsOnMap()
    ...
    --have they selected to only use it on node maps?
    if TFC.settings.frameOnBaselessMaps then
        --is this map nodeless?
        self:getPOIs()
        if not POIList and next(POIList) == nil then return end
    end
...
function addon:showClassBlip(parentFrame, player, x, y, faction, playerNum)
    if parentFrame["TFCBlipTexture"..faction..playerNum] == nil then
        parentFrame["TFCBlipTexture"..faction..playerNum] = parentFrame:CreateTexture("TFCBlipTexture"..faction..playerNum)
        parentFrame["TFCBlipTexture"..faction..playerNum]:SetTexture("Interface\\Addons\\REPorter\\Textures\\BlipNormal")
    end
    local texture = parentFrame["TFCBlipTexture"..faction..playerNum]
    texture:SetPoint("CENTER", parentFrame, x, y)
    texture:SetWidth(10)
    texture:SetHeight(10)
    -- SetPortraitToTexture(texture, "Interface\\Addons\\REPorter\\Textures\\BlipNormal")
...
function addon:getUnitDetails(unit)
    if not UnitExists(unit) or UnitIsUnit(unit,'player') or not UnitIsPlayer(unit) or (UnitReaction(unit, "player") == nil) then
        return
    end
    ...
    details['fullName'] = self:getFullName(unit)
    ...
    if details['name'] == 'Unknown' or details['name'] == 'Unbekannt' then return nil end
   
    return details
end

function addon:getFullName(fullName)
    if UnitExists(fullName) then
    ...
    -- self:Debug("Into", string.gsub(fullName, "%s+", ""))
...
function addon:NAME_PLATE_UNIT_ADDED(unit)
    ...
    --Ignore if the unit is not in interaction range. IE, they are not in teamfight, we are simply targeting them with BGE.
    if not frame or frame['isTarget'] and not frame['isClose'] then return end
...
function addon:NAME_PLATE_UNIT_REMOVED(unit)
    ...
    -- self:Debug('Frame Removed', unit, frame.fullName)
...
function addon:PLAYER_ENTERING_WORLD()
    local fullName = addon:getFullName('player')
    counters = {}
...
function addon:countNearbyFactions()
    ...
    displayFrame.displayEnemy:SetText('('..(friendly+1)..'v'..enemy..')')
